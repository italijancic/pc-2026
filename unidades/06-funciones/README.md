...
- **Taller en clase** — cuatro funciones para escribir de cero: aleatorios y primos
...
[presentación](./presentacion.pdf) (diapositivas 37 a 44). No se entregan: se resuelven ahí y
se corrigen en el proyector.

| | Consigna | Firma |
|--|----------|-------|
| **A1** | Un decimal aleatorio en un rango | `rndFloat(min, max)` |
| **A2** | Un entero par o impar, a elección | `rndParity(min, max, wantEven)` |
| **B1** | ⭐ ¿es primo? | `isPrime(n)` |
| **B2** | Un primo al azar de un rango | `rndPrime(min, max)` |

Los bloques **A** y **B** son independientes. Dentro de **B** sí hay orden: `isPrime` es la
pieza, y `rndPrime` la usa sin volver a escribir nada de primos.

> 🔑 Es el punto de la unidad: mirá `rndPrime`. **No tiene una sola línea de primos ni
> de sorteo** — llama a `rndInt` y a `isPrime`, que ya andaban. Eso es componer.
