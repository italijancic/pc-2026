...
<div class="pitfall">
<p><strong>Qué se puede devolver:</strong> un valor primitivo — número, texto, booleano — y desde la unidad 07, un vector. <strong>Objetos y JSON no.</strong> Por ahora, si necesitás devolver tres cosas, son tres funciones; desde la unidad 07 pueden viajar juntas en un vector.</p>
</div>
...
<p class="eyebrow"><b>06</b><span>/</span>Taller</p>
...
<ol class="enum">
<li><span>Respetá <strong>la firma exacta</strong> que pide cada consigna. Los nombres son los del curso.</span></li>
<li><span><strong>JSDoc en todas.</strong> Sin excepción.</span></li>
<li><span>Escribí la función <strong>y</strong> el <code>console.log</code> que la prueba.</span></li>
<li><span>Probá con <strong>casos límite</strong>: el mínimo, el máximo, el 1, el 0.</span></li>
<li><span>Una función que anda se deja quieta: la siguiente <strong>la va a usar</strong>.</span></li>
</ol>

</div>

<!-- Cuatro consignas. A y B son independientes: si una pareja se traba en A2,
     que pase a B1 igual. B1 es la importante, y B2 se apoya en ella.
     Sugerido: 10 min para A, 20 min para B, y corregimos en el proyector. -->

---

<p class="eyebrow"><b>06</b><span>/</span>Taller A</p>

## A1 · Un decimal aleatorio

<div class="body">

<p class="lead">Un sensor de temperatura no devuelve enteros. Escribí <code>rndFloat(min, max)</code>, que dé un <strong>decimal</strong> aleatorio dentro del rango.</p>
...
console.log(rndFloat(20, 30).toFixed(2))
```
...
<p class="note-p">Arrancá de <code>rndInt</code> y <strong>sacale lo que sobra</strong>.</p>

</div>

<!-- Respuesta: Math.random() * (max - min) + min
     Sin floor no hace falta el + 1: no hay redondeo que se coma el extremo.
     Preguntar por qué. El max exacto no sale nunca — el intervalo es [min, max). -->

---

<p class="eyebrow"><b>06</b><span>/</span>Taller A</p>

## A2 · Par o impar, a elección

<div class="body">

<p class="lead"><code>rndParity(min, max, wantEven)</code> devuelve un entero del rango: <strong>par</strong> si <code>wantEven</code> es <code>true</code>, <strong>impar</strong> si es <code>false</code>.</p>

<div class="file" data-name="src/app.js">

```js
const parity = prompt('¿Par o impar? ')

console.log(rndParity(10, 20, parity === 'par'))
```
...
<p class="note-p">Es la primera función del curso con un <strong>parámetro booleano</strong>. Mirá cómo lo arma el programa principal.</p>

</div>

<!-- La vía directa: sortear con rndInt y repetir mientras la paridad no sea la
     pedida — do-while de la unidad 05. Ojo con rndParity(3, 3, true): no termina.
     Hay una versión sin reintentos, está en el archivo de la corrección. -->
...
## ¿Cómo hace un programa para saber si un número es primo?

<p class="lead">Primo: divisible sólo por <strong>1</strong> y por <strong>sí mismo</strong>. El 1 no es primo.</p>

<!-- Que lo digan en voz alta antes de escribir nada. La respuesta es «probar
     divisores», y probar es un bucle. Es el momento de la clase donde se juntan
     la unidad 05 y la 06. -->

---

<p class="eyebrow"><b>06</b><span>/</span>Taller B</p>

## B1 · <code>isPrime(n)</code>

<div class="body">

<p class="lead">Devuelve <code>true</code> si <code>n</code> es primo y <code>false</code> si no. <strong>Es la pieza</strong>: la consigna que sigue se apoya en ella.</p>
...
<div class="tip">
<p>Un bucle que prueba divisores. En cuanto encontrás uno, <code>return false</code> — y el bucle <strong>se corta solo</strong>.</p>
</div>

</div>

<!-- Acá se paga la diapositiva «devuelve Y termina». No hace falta ninguna
     variable bandera: el return sale de la función desde adentro del for.
     Los tres casos del ejemplo son los tres que fallan: 7 primo, 9 compuesto
     impar (el que se escapa si probás sólo la división por 2), y el 1. -->
...
<p class="lead"><code>rndPrime(min, max)</code> devuelve un entero <strong>primo</strong> aleatorio del rango. No la escribas de cero: ya tenés las dos piezas.</p>

<div class="file" data-name="src/app.js">

```js
const rndPrime = (min, max) => // ...

console.log(rndPrime(1, 50))
```
...
<div class="pitfall">
<p>¿Qué pasa si en el rango <strong>no hay ningún primo</strong>? Probá <code>rndPrime(24, 28)</code>.</p>
</div>

</div>

<!-- 24, 25, 26, 27 y 28: ninguno es primo. El programa se cuelga y hay que
     cortarlo con Ctrl+C. Es el bucle infinito de la unidad 05, pero escondido
     adentro de una función que «anda bien» en todas las demás pruebas. -->
...
<ol class="enum">
<li><span><code>rndParity</code> usa <code>rndInt</code></span></li>
<li><span><code>rndPrime</code> usa <code>rndInt</code> <strong>y</strong> <code>isPrime</code></span></li>
</ol>

<p class="callout">Mirá <code>rndPrime</code>: no tiene <strong>una sola línea</strong> de primos ni de sorteo. <small>Llama a dos funciones que ya andaban. Eso es componer, y es de lo que están hechos los programas grandes.</small></p>

</div>

<!-- Cerrar con esto: nadie escribió «el algoritmo de los primos» dos veces.
     Se escribió una vez, en isPrime, y rndPrime la llama sin saber cómo está
     hecha por dentro.

     SI SOBRA TIEMPO, la de reserva es printPrimesUpTo(limit): imprime todos
     los primos hasta el límite. Un for del 2 al limit y un if con isPrime
     adentro — dos líneas, y es una función que NO devuelve nada. Está
     resuelta en el archivo de la corrección. Si preguntan por qué imprime en
     vez de devolver la lista: porque todavía no tenemos dónde guardarla.
     Eso es la unidad 07. -->
...
<li><span>Una función puede <strong>usar otra</strong>. Así se arman los programas grandes.</span></li>
</ol>
